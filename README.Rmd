...
library(tidyverse)
rbx$rby %>% 
  select(year, rec = r, bio, ssb, fbar, hr, oY) %>% 
...
d %>% 
  ...
  geom_vline(xintercept = 2017.5, colour = "grey") +
...
save(list=c("HCRsettings","alldata","sumdata"), file = "HCRrun.rdata")
save(list=c("HCRsettings","sumdata"), file = "HCRrunsum.rdata")
...
```{r, fig.heigth = 9}
load("0Base/HCRrun.rdata")
d <-
  alldata %>%
  as_tibble() %>%
  select(hr, Btrigger, year,
         catch, ssb, rec, fbar = refF, bio = refbio1) %>%
  mutate(rec = rec / 1000) %>%
...
d %>%
  ...
  ggplot(aes(year)) +
  theme_bw() +
  geom_vline(xintercept = 2018.5, colour = "grey") +
